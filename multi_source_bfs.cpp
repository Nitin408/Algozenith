...
void bfs(vector<state> allst){
    ...
    while(!q.empty()){
        ...
        for(int k=0; k<8; k++){
            state v = {cur.ff+dx[k], cur.ss+dy[k]};
...
void solve(){
  ...
  int k;cin>>k;
  vector<state> st(k);
  for(int i=0;i<k;i++)cin>>st[i].ff>>st[i].ss;
  
  state en; 
  cin>>en.ff>>en.ss;
  bfs(st);

  cout<<"distance "<<dist[en.ff][en.ss]<<nl;

  vector<state> path;
  state okay = {-1,-1};
  while(en != okay){              // en != make_pair(-1,-1)
    path.push_back(en);
    en=back[en.ff][en.ss];
  }

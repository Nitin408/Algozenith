...
void dijkstra(int sc){
    ...
    dist[sc]=0;
    priority_queue<pair<int,int>, vector<pair<int,int>>, prioritize> pq;   // storing {node, distance of that node from sc} 
    pq.push({sc,0});                                                        // in priority queue.
    ...
    while(!pq.empty()){
        ...
        for(auto v: g[fs.first]){
            ...
            int weight = v.second;
            if(dist[neighbour]>dist[fs.first]+weight){
                dist[neighbour] = dist[fs.first]+weight;
